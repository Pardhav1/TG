...
function formatTripDetails(aiResponse, tripData) {
    try {
        let textContent = typeof aiResponse === 'object'
        ...
        return `
            <div class="itinerary-container">
                <div class="trip-header">
                    <h1><i class="fas fa-map-marked-alt"></i> ${escapeHtml(tripData.destination || 'Your Trip')} Itinerary</h1>
                    <div class="trip-meta">
                        <span><i class="fas fa-calendar-alt"></i> ${escapeHtml(tripData.startDate || 'Start date')} to ${escapeHtml(tripData.endDate || 'End date')}</span>
                        ${tripData.customization ? `<span><i class="fas fa-star"></i> ${escapeHtml(tripData.customization)}</span>` : ''}
                    </div>
                </div>
...
function formatTextResponse(text) {
    try {
        if (!text) return '<p class="no-content">No itinerary content available.</p>';
        text = text.replace(/\r\n/g, '\n').replace(/\n+/g, '\n').trim();
        ...
        return hasDays ? formatDayByDayItinerary(text) : formatGeneralItinerary(text);
    } catch (error) {
...
function formatDayByDayItinerary(text) {
    try {
        ...
        for (let i = 1; i < days.length; i += 2) {
            ...
            let dayContent = days[i + 1] ? days[i + 1].trim().replace(/^[:\-\s]+/, '') : '';
...
function formatDayContent(content) {
    try {
        const sections = content.split(/(📍|🏨|🍽|💡|Places to Visit|Stay|Food Recommendations|Extra Tips)/i);
        let html = '';

        for (let i = 1; i < sections.length; i += 2) {
            const sectionTitle = sections[i]?.trim();
            let sectionContent = sections[i + 1]?.trim().replace(/^[:\-\s]+/, '') || '';
            ...
            html += `
                <div class="section ${type}">
                    <div class="section-header">
                        ...
                        <h3>${escapeHtml(getFormattedSectionTitle(sectionTitle, type))}</h3>
                    </div>
        ...
        return html || `<div class="general-content">${escapeHtml(content)}</div>`;
    } catch (error) {
...
function formatPlacesContent(content) {
    try {
        ...
        for (let i = 1; i < timeBlocks.length; i += 2) {
            ...
            let timeContent = timeBlocks[i + 1]?.trim().replace(/^[:\-\s]+/, '') || '';
            ...
            html += `
                <div class="time-block">
                    <div class="time-header">
                        ...
                        <h4>${escapeHtml(getFormattedTimeTitle(timeTitle, timeIcon))}</h4>
                    </div>
...
function getSectionType(title) {
    ...
    if (lower.includes('place') || lower.includes('visit') || title === '📍') return { type: 'places', icon: '📍' };
    if (lower.includes('stay') || lower.includes('hotel') || title === '🏨') return { type: 'stay', icon: '🏨' };
    if (lower.includes('food') || lower.includes('eat') || lower.includes('dining') || title === '🍽') return { type: 'food', icon: '🍽' };
    if (lower.includes('tip') || lower.includes('note') || title === '💡') return { type: 'tips', icon: '💡' };
...
function getFormattedSectionTitle(title, type) {
    if (!title || title.length <= 2) {
        switch (type) {
            ...
            case 'food': return 'Food Suggestions';
...
function formatGeneralItinerary(text) {
    ...
    window.downloadPDF = function() {
        ...
        html2canvas(tripDetailsDiv, options).then(canvas => {
